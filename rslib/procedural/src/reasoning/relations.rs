...
impl BloodRelationPuzzle {
    ...
    pub fn create_grandmother_chain(person_a: &str, person_b: &str, person_c: &str) -> Self {
        let s1 = KinshipStatement::new(person_a, KinshipRelation::Mother, person_b);
        let s2 = KinshipStatement::new(person_b, KinshipRelation::Father, person_c);

        let explanation = format!(
            "{} is the mother of {}. Since {} is the father of {}, {} is the grandmother of {}.",
            person_a, person_b, person_b, person_c, person_a, person_c
        );
        ...
        Self {
            ...
            query_from: person_a.to_string(),
            query_to: person_c.to_string(),
            target_relation: KinshipRelation::Grandmother,
            explanation,
        }
    }

    pub fn create_niece_chain(person_a: &str, person_b: &str, person_c: &str) -> Self {
        let s1 = KinshipStatement::new(person_b, KinshipRelation::Brother, person_a);
        let s2 = KinshipStatement::new(person_c, KinshipRelation::Daughter, person_b);

        let explanation = format!(
            "{} is the brother of {}. Since {} is the daughter of {}, {} is the niece of {}.",
            person_b, person_a, person_c, person_b, person_c, person_a
        );
        ...
        Self {
            ...
            query_from: person_c.to_string(),
            query_to: person_a.to_string(),
            target_relation: KinshipRelation::Niece,
            explanation,
        }
    }

    pub fn create_multi_hop_chain(person_a: &str, person_b: &str, person_c: &str, person_d: &str) -> Self {
        let s1 = KinshipStatement::new(person_a, KinshipRelation::Father, person_b);
        let s2 = KinshipStatement::new(person_b, KinshipRelation::Brother, person_c);
        let s3 = KinshipStatement::new(person_c, KinshipRelation::Father, person_d);

        let explanation = format!(
            "{} is the father of {}. {} is the brother of {}. {} is the father of {}. Therefore, {} is the grandfather of {}.",
            person_a, person_b, person_b, person_c, person_c, person_d, person_a, person_d
        );
...
impl DirectionPuzzle {
    fn compute_heading(dx: i32, dy: i32) -> Heading {
    ...
    pub fn create_2step_path(d1: i32, d2: i32) -> Self {
        let steps = vec![
    ...
    pub fn create_4step_path(d1: i32, d2: i32, d3: i32, d4: i32) -> Self {
        let steps = vec![
            format!("Walks {} meters North.", d1),
            format!("Turns right and walks {} meters East.", d2),
            format!("Turns right and walks {} meters South.", d3),
            format!("Turns left and walks {} meters East.", d4),
        ];
        ...
        let explanation = format!(
            "Net displacement: \\(\\Delta x = {} + {} = {} \\text{{ m East}}, \\Delta y = {} - {} = {} \\text{{ m North}}\\).\n\
             Straight-line distance = \\(\\sqrt{{({})^2 + ({})^2}} = **{:.1} meters** in the **{}** direction.",
            d2, d4, dx, d1, d3, dy, dx, dy, dist, heading.as_str()
        );
    ...
    pub fn create_5step_path(d1: i32, d2: i32, d3: i32, d4: i32, d5: i32) -> Self {
        let steps = vec![
            format!("Walks {} meters North.", d1),
            format!("Turns right and walks {} meters East.", d2),
            format!("Turns right and walks {} meters South.", d3),
            format!("Turns right and walks {} meters West.", d4),
            format!("Turns right and walks {} meters North.", d5),
        ];
        ...
        let explanation = format!(
            "Net displacement: \\(\\Delta x = {} - {} = {} \\text{{ m}}, \\Delta y = {} - {} + {} = {} \\text{{ m}}\\).\n\
             Straight-line distance = \\(\\sqrt{{({})^2 + ({})^2}} = **{:.1} meters** in the **{}** direction.",
            d2, d4, dx, d1, d3, d5, dy, dx, dy, dist, heading.as_str()
        );
