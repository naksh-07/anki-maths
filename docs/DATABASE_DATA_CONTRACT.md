# StudyLab Database Schema & Data Ownership Contract
## SQLite Architecture, Table DDLs, Migration Catalog & Data Durability Taxonomy

**Document Version:** 1.0.0 (Canonical)  
**Target Subsystems:** Rust Storage Layer (`rslib/procedural/src/storage/`), Migration Engine (`migration.rs`), SQLite Store (`procedural.db` / `<collection>.procedural`)  
**Status:** AUTHORITATIVE CANONICAL SPECIFICATION  
**Integrity Mode:** 100% Grounded in Executable Source Code, Rust Schemas, and Migration Test Suites  

---

## 1. Architectural Overview & Database Separation Invariant

StudyLab strictly enforces physical and logical database separation between Anki's native collection store (`collection.anki2`) and StudyLab's procedural learning store (`<collection_name>.procedural` or `procedural.db`).

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│                               DATABASE BOUNDARY SEPARATION                             │
├────────────────────────────────────────┬───────────────────────────────────────────────┤
│ ANKI NATIVE STORE                      │ STUDYLAB PROCEDURAL STORE                     │
│ (`collection.anki2`)                   │ (`<collection>.procedural` / `procedural.db`) │
├────────────────────────────────────────┼───────────────────────────────────────────────┤
│ • Notes, Cards, Decks, Models, Configs │ • Cognitive Skill Graph & EMA Mastery States  │
│ • FSRS / SM-2 Spaced Repetition Revlog │ • Problem Families & Declarative Schemas      │
│ • Ephemeral `ProceduralCardAnchor` JSON│ • Concrete Problem Instances & Derivations    │
│ • Standard Unmodified Anki Schema      │ • Practice Attempts & Diagnostic Error Events │
│ • 100-byte `cards.data` column limit   │ • PYQ Source Ingestion & Exam Profiles        │
│ • Synchronized via AnkiWeb Channel     │ • Canonical Practice Items & Chapter Profiles │
│                                        │ • Durable Remediation Queue & Circuit Breakers│
└────────────────────────────────────────┴───────────────────────────────────────────────┘
```

### 1.1 The 4 Architectural Invariants of Database Separation
1. **Zero Core Schema Pollution:** StudyLab never adds, modifies, or deletes tables in Anki's upstream `collection.anki2` schema. Standard Anki upgrades will never experience schema conflicts or data corruption from StudyLab.
2. **AnkiWeb Sync Channel Protection:** Standard AnkiWeb syncs `collection.anki2` and media collections. Decoupling high-frequency procedural telemetry avoids saturating the sync bandwidth or violating Anki's **100-byte custom data limit** on `cards.data`.
3. **Independent Schema Evolution:** Procedural mastery tracking requires rich relational modeling (16 tables, 22 indexes, directed acyclic dependency graphs, JSON diagnostic signals). An independent SQLite store permits rapid schema migration (v1 through v5) without affecting collection stability.
4. **Targeted ACID Transaction Boundaries:** High-frequency student attempt logs, exponential moving average updates, error category attributions, and remediation queues are committed in dedicated atomic SQLite transactions without acquiring long locks on Anki's main collection database.

---

## 2. Storage Engine & SQLite Operational Pragmas
...
Upon initializing the SQLite database connection, `ProceduralStore::apply_pragmas` executes five mandatory high-performance pragmas:
...
### 2.1 Pragma Rationale & Operational Guarantees
- **`busy_timeout = 5000`:** Configures a 5,000-millisecond lock acquisition timeout. Prevents transient `SQLITE_BUSY` errors during concurrent background task execution.
- **`foreign_keys = ON`:** Strictly enforces relational constraints and cascading deletes (`ON DELETE CASCADE`) across skills, families, schemas, instances, and attempt logs.
- **`synchronous = NORMAL`:** Optimizes disk I/O in WAL mode while maintaining full ACID crash resilience across unexpected power failures or OS crashes.
- **`temp_store = MEMORY`:** Executes temporary sorting buffers, transient index builds, and join filters entirely in RAM for sub-millisecond query evaluation.
- **`journal_mode = WAL`:** Write-Ahead Logging allows non-blocking concurrent readers while background transactional writes are committed.

---

## 3. Migration Runner & Versioning History (v1 to v5)

Database schema evolution is managed by `MigrationRunner` (`rslib/procedural/src/storage/migration.rs`). The runner inspects the dedicated `schema_migrations` tracking table:
...
## 4. Comprehensive SQLite Table DDL Specifications

The procedural database comprises **16 production tables** organized into 6 functional clusters:
...
```json
{
  "progression_state": "Transfer",
  "recent_outcomes": [1.0, 1.0, 0.0, 1.0, 1.0],
...
CREATE TABLE IF NOT EXISTS remediation_queue_items (
    ...
    kind TEXT NOT NULL,             -- Action kind: "ConceptCheck", "StrategyDrill", "WorkedExample", "PrerequisiteReview", "CircuitBreaker"
    skill_id TEXT NOT NULL,         -- Target SkillId
    schema_id TEXT NOT NULL,        -- Target SchemaId
    domain TEXT NOT NULL,           -- Academic domain
...
## 5. Comprehensive Index Catalog (22 Indexes)

The procedural store maintains **22 targeted indexes** optimizing relationship traversal, temporal ordering, and queue prioritization:

| # | Index Name | Target Table | Indexed Columns | Query Optimization Impact |
|---|---|---|---|---|
| 1 | `idx_skills_domain` | `skills` | `(domain)` | Accelerates domain filtering during diagnostic testing. |
| 2 | `idx_families_skill` | `problem_families` | `(skill_id)` | Optimizes foreign key joins from skills to problem families. |
| 3 | `idx_schemas_skill` | `schemas` | `(skill_id)` | Resolves active schemas for a specific skill node. |
| 4 | `idx_instances_family` | `problem_instances` | `(family_id)` | Traverses generated instances per family. |
| 5 | `idx_attempts_schema` | `practice_attempts` | `(schema_id)` | Calculates schema-level accuracy metrics. |
| 6 | `idx_attempts_skill` | `practice_attempts` | `(skill_id)` | Computes skill-level mastery and sliding windows. |
| 7 | `idx_attempts_card` | `practice_attempts` | `(card_id)` | Links Anki card review requests to procedural history. |
| 8 | `idx_attempts_time` | `practice_attempts` | `(attempted_at)` | Temporal ordering for retention and latency tracking. |
| 9 | `idx_error_events_attempt` | `error_events` | `(attempt_id)` | Cascading retrieval of error diagnostics per attempt. |
| 10 | `idx_skill_states_updated` | `skill_states` | `(updated_at)` | Identifies stale skills for spaced review sweeps. |
| 11 | `idx_pyq_sources_exam` | `pyq_sources` | `(exam)` | Filters authentic questions by target exam profile. |
| 12 | `idx_pyq_sources_domain` | `pyq_sources` | `(domain)` | Filters PYQ sources across academic domains. |
| 13 | `idx_pyq_mappings_schema` | `pyq_mappings` | `(schema_id)` | Resolves PYQ templates for a practice schema. |
| 14 | `idx_pyq_mappings_status` | `pyq_mappings` | `(status)` | Selects verified PYQ mappings for production decks. |
| 15 | `idx_pyq_mappings_confidence` | `pyq_mappings` | `(confidence)` | Prioritizes high-confidence mappings. |
| 16 | `idx_rejected_variants_pyq` | `rejected_variants` | `(source_pyq_id)` | Audits defect generation per source PYQ. |
| 17 | `idx_rejected_variants_family` | `rejected_variants` | `(family_id)` | Identifies problem families with generator defects. |
| 18 | `idx_practice_items_schema` | `practice_items` | `(schema_id)` | Ingestion retrieval of practice items per schema. |
| 19 | `idx_practice_items_family` | `practice_items` | `(problem_family_id)` | Ingestion retrieval of practice items per family. |
| 20 | `idx_practice_items_chapter` | `practice_items` | `(chapter)` | Groups practice items by chapter for batch export. |
| 21 | `idx_remediation_queue_skill` | `remediation_queue_items` | `(skill_id)` | Enables same-skill queue compaction. |
| 22 | `idx_remediation_queue_urgency`| `remediation_queue_items` | `(urgency)` | Priority queue extraction of critical remediation items. |

---

## 6. Data Classification & Lifecycle Model

StudyLab data is rigorously classified into four distinct durability categories:

```text
┌────────────────────────────────────────────────────────────────────────────────────────┐
│                              DATA CLASSIFICATION TAXONOMY                              │
├───────────────────┬───────────────────────────────────┬────────────────────────────────┤
│ Category          │ Target Tables & Entities          │ Durability & Storage Policy    │
├───────────────────┼───────────────────────────────────┼────────────────────────────────┤
│ **Durable Data**  │ `skills`, `skill_states`,         │ **Permanent Store:**           │
│                   │ `practice_attempts`,              │ Never purged; forms the core   │
│                   │ `error_events`, `pyq_sources`,    │ longitudinal learning history  │
│                   │ `remediation_queue_items`         │ and student cognitive profile. │
├───────────────────┼───────────────────────────────────┼────────────────────────────────┤
│ **Derived Data**  │ EMA Mastery ($M_t$),              │ **Statistically Computed:**    │
│                   │ Moving Latency Stats,             │ Updated via atomic equations;  │
│                   │ Speed Quadrants,                  │ fully recomputable from raw    │
│                   │ Progression States (6 Gates)      │ `practice_attempts` history.   │
├───────────────────┼───────────────────────────────────┼────────────────────────────────┤
│ **Cached Data**   │ `catalog_metadata`,               │ **Query Acceleration Cache:**  │
│                   │ `chapter_practice_profiles`,      │ May be dropped or rebuilt      │
│                   │ Prerequisite Graph In-Memory Cache│ without losing student state.  │
├───────────────────┼───────────────────────────────────┼────────────────────────────────┤
│ **Reproducible**  │ `problem_instances`               │ **Deterministic Ephemeral:**   │
│ **Data**          │ (rendered prompts, parameters)    │ Generated dynamically on review│
│                   │                                   │ from `(archetype, seed)`.      │
└───────────────────┴───────────────────────────────────┴────────────────────────────────┘
```
...
## 7. Atomic Transaction Lifecycles & Concurrency

All mutative operations in the procedural store execute inside strict SQLite transaction blocks to prevent partial writes and database corruption.

### 7.1 Atomic Practice Attempt Ingestion (`record_practice_attempt_atomic`)
Implemented in `ProceduralStore::record_practice_attempt_atomic` (`store.rs:743-890`):
...
```text
 ...
 3. In-Memory EMA Mastery & Signal Processing:
    ...
    - Confidence_t = min(TotalAttempts / 10.0, 1.0)
    - Update MovingLatencyStats & ErrorFrequencyCounts
    - Evaluate 6-Gate Progression State Promotion/Demotion
 4. INSERT INTO practice_attempts (id, instance_id, schema_id, skill_id, card_id, user_answer, is_correct, score, time_taken_ms, attempted_at, metadata)
 5. INSERT INTO error_events (for each diagnostic error event)
 6. UPSERT INTO skill_states (skill_id, mastery, confidence, total_attempts, successful_attempts, last_practiced_at, custom_state, updated_at)
 7. COMMIT TRANSACTION (tx.commit()?)
```

If any step encounters an error (e.g. disk quota exhaustion or constraint violation), the entire transaction rolls back cleanly, ensuring that attempt logs and skill states never desynchronize.

---

## 8. Parameterized Queries & Security Defenses

To completely prevent SQL injection vulnerabilities:
- **100% Positional Placeholders:** All SQL queries in `rslib/procedural/src/storage/` use positional parameter placeholders (`?1, ?2, ...`) or `rusqlite::params![...]`.
- **Zero String Concatenation:** Raw strings or student inputs are never interpolated directly into SQL command strings.
- **Strict Serde Serialization:** Complex metadata and JSON fields are strictly serialized via `serde_json::to_string()` prior to parameter binding.
- **Cascading Referential Integrity:** All relational foreign keys declare `ON DELETE CASCADE`, guaranteeing clean tree deletion when skills or schemas are pruned.

---

## 9. Summary & Acceptance Checklist

| Component | Standard Specification | Verification Evidence |
|---|---|---|
| **Database Separation** | `collection.anki2` vs `procedural.db` | Verified in `store.rs` & `render.rs` |
| **Pragmas** | WAL mode, foreign keys ON, busy timeout 5000 | Verified in `store.rs:36-42` |
| **Migration Engine** | 5 migrations (v1-v5) tracked in `schema_migrations` | Verified in `migration.rs:10-78` |
| **Table DDLs** | 16 tables across skills, schemas, attempts, exams, queue | Verified in `schema.rs:10-283` |
| **Index Coverage** | 22 dedicated query optimization indexes | Verified in `schema.rs:102-280` |
| **Atomic Transactions** | Single-transaction attempt & mastery commit | Verified in `store.rs:743-890` |
| **Data Classification** | Durable vs Derived vs Cached vs Reproducible | Fully documented in Section 6 |
| **Injection Defense** | 100% parameterized bindings with `rusqlite` | Verified across storage layer |
